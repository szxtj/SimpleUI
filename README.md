...
### 📚 本地离线维基知识库 (Offline Wiki RAG)
- 原生对接本地 Kiwix (ZIM) 离线维基服务，主界面与设置面板实时呈现服务状态与词条规模。
- 采用轻量小模型进行智能意图规划与实体关键词抽取，结合前缀联想、段落精排与简繁自动转换，将高关联知识无缝注入大模型上下文。
...
SimpleUI 原生内置对本地离线维基（如 Kiwix 运行的 ZIM 词库）的免配置检索增强（RAG）支持：
- **服务对接**：设置面板支持配置本地维基端口（默认 `8080`），主界面右下角实时呈现服务在线状态与词条收录规模。
- **智能意图规划**：优先调用本地 2B 轻量小模型规划检索实体与别名，未命中时平滑降级至关键词联想与段落精排；集成 OpenCC 简繁自动转换。
- **无感上下文注入**：高相关度词条内容自动组装至上下文提示词中提交给主推理模型，实现 100% 离线、高精准的知识问答。
